...
import { executeRecentTradeMomentumV1 } from "../../../../../../domain/trade-analytics/recent-trade-momentum-v1-adapter";
...
type RecentTradeMomentumRouteContext = {
  params: Promise<{
...
export async function GET(
  request: Request,
  context: RecentTradeMomentumRouteContext,
): Promise<Response> {
  return recentTradeMomentumRoute.get(request, context);
}
...
export async function HEAD(
  ...
  return recentTradeMomentumRoute.head(request, context);
}

const recentTradeMomentumRoute =
  createMeasuredRuntimeRoute<RecentTradeMomentumRouteContext>({
    routeFamily: "recent-trade-momentum",
    deadlineMs: ROUTE_DEADLINE_MS.recentTradeMomentum,
    async respond({
      request,
      context,
      runtime,
      ...
      const result = await executeRecentTradeMomentumV1(
        runtime.tradeAnalytics,
        {
          analysisBuildId,
          reporterCode: url.searchParams.get("reporter") ?? "",
          productCode: url.searchParams.get("product") ?? "",
          exporterCode: url.searchParams.get("exporter") ?? undefined,
        },
        {
          signal,
    ...
    errorResponse(error, measurement) {
      ...
      writeStructuredErrorLog(
        "recent-trade-momentum-analysis-request-failed",
        error,
        { correlationId },
      );
      return jsonErrorResponse(
        500,
        "INTERNAL_ERROR",
        "Recent Trade Momentum analysis could not be completed.",
        correlationId,
      );
    },
  });
...
function validateSearchParameters(searchParameters: URLSearchParams): void {
  ...
  if (
    ...
    searchParameters.getAll("reporter").length !== 1 ||
    searchParameters.getAll("product").length !== 1 ||
    searchParameters.getAll("exporter").length > 1 ||
    keys.some(
      (key) => key !== "reporter" && key !== "product" && key !== "exporter",
    )
  ) {
    throw invalidRecentTradeMomentumQuery(
      "The route accepts one reporter, one product, and at most one exporter parameter.",
    );
  }
}
