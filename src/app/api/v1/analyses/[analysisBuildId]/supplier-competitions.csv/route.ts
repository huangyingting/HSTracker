...
} from "../../../../../../export/supplier-competition-csv";
import type { SupplierCompetitionCsvIdentity } from "../../../../../../export/supplier-competition-csv-contract";
...
type SupplierCompetitionExportRouteContext = {
  params: Promise<{
    analysisBuildId: string;
  }>;
};

class SupplierCompetitionExportRouteError extends Error {
  constructor(
    ...
    super(message);
    this.name = "SupplierCompetitionExportRouteError";
  }
}

export async function GET(
  request: Request,
  context: SupplierCompetitionExportRouteContext,
): Promise<Response> {
  return supplierCompetitionCsvRoute.get(request, context);
}
...
export async function HEAD(
  ...
  return supplierCompetitionCsvRoute.head(request, context);
}

const supplierCompetitionCsvRoute =
  createMeasuredRuntimeRoute<SupplierCompetitionExportRouteContext>({
    routeFamily: "supplier-competition-csv",
    deadlineMs: ROUTE_DEADLINE_MS.supplierCompetitionCsv,
    async respond({
      request,
      context,
      runtime,
      ...
      const identity: SupplierCompetitionCsvIdentity = {
        analysisBuildId,
        ...parsedIdentity,
      };
      ...
      if (
        ...
        throw new SupplierCompetitionExportRouteError(
          410,
    ...
    errorResponse(error, measurement) {
      ...
      if (error instanceof SupplierCompetitionExportRouteError) {
      ...
      if (error instanceof SupplierCompetitionCsvRepresentationError) {
        return jsonErrorResponse(
          503,
          error.code,
          "The complete Supplier Competition export is temporarily unavailable.",
        );
      }

      const correlationId = measurement.correlationId;
      writeStructuredErrorLog(
        "supplier-competition-csv-export-request-failed",
        error,
        { correlationId },
      );
      return jsonErrorResponse(
        500,
        "INTERNAL_ERROR",
        "Supplier Competition export could not be completed.",
        correlationId,
      );
    },
  });
...
function validateSearchParameters(
...
): Omit<SupplierCompetitionCsvIdentity, "analysisBuildId"> {
  ...
  if (schema !== SUPPLIER_COMPETITIONS_CSV_SCHEMA_VERSION) {
    throw new SupplierCompetitionExportRouteError(
      400,
  ...
  if (!/^\d{1,3}$/u.test(importer)) {
    throw invalidSupplierCompetitionQuery(
      "The importer must be a BACI economy code.",
    );
  }
  if (!/^\d{6}$/u.test(product)) {
    throw invalidSupplierCompetitionQuery(
      "The product must be a six-digit HS12 code.",
    );
  }
...
function validateIdentifier(value: string, name: string): void {
  if (!/^[A-Za-z0-9][A-Za-z0-9._:%-]*$/u.test(value)) {
    throw invalidSupplierCompetitionQuery(`The ${name} identity is malformed.`);
  }
}
