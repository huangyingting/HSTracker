...
} from "./trade-trend-export-client";

const copy = {
  en: {
    region: "Trade Trend Result Export",
    eyebrow: "Trade Trend Result Export",
    scope:
      "The complete five-Finalized-Year series plus the Provisional Year snapshot.",
    detail:
...
class TradeTrendCsvDownloadError extends Error {
  constructor(
    readonly stale: boolean,
    message: string,
  ) {
    super(message);
    this.name = "TradeTrendCsvDownloadError";
  }
}

export function TradeTrendExportAction({
  result,
  locale,
  ...
  useEffect(() => {
    ...
    void downloadCsv(pendingUrl, controller.signal)
      ...
      .catch(async (error: unknown) => {
        ...
        if (error instanceof TradeTrendCsvDownloadError) {
          if (error.stale) {
          ...
          console.error("Trade Trend CSV download failed", error);
          setStatus("failed");
        }
      })
      .finally(() => {
  ...
  async function beginExport() {
    ...
    try {
      ...
      if (error instanceof TradeTrendExportPreparationError) {
        onManifestRevalidated(error.manifest);
        setStatus("stale");
      } else {
        console.error("Trade Trend export preflight failed", error);
        setStatus("failed");
      }
    } finally {
...
async function revalidateAfterDownloadConflict(
  ...
  try {
    ...
    console.error(
      "Trade Trend export conflict revalidation failed",
      error,
    );
  }
}
...
async function downloadCsv(url: string, signal: AbortSignal): Promise<void> {
  ...
  if (!response.ok) {
    throw new TradeTrendCsvDownloadError(
      response.status === 404 ||
        response.status === 409 ||
        response.status === 410,
      `Trade Trend CSV returned ${response.status}.`,
    );
  }
  ...
  if (
    ...
    throw new TradeTrendCsvDownloadError(
      false,
      "Trade Trend CSV returned invalid representation metadata.",
    );
  }
