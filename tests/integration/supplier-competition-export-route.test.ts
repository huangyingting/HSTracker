...
import {
  GET,
  HEAD,
} from "../../src/app/api/v1/analyses/[analysisBuildId]/supplier-competitions.csv/route";
...
import { serializeSupplierCompetitionCsv } from "../../src/export/supplier-competition-csv";
...
describe("versioned Supplier Competition CSV route", () => {
  it("serves matching deterministic GET, conditional GET, and HEAD metadata", async () => {
    ...
    const platformCsv = serializeSupplierCompetitionCsv({
      result: {
        ...outcome.payload,
    ...
    expect(first.headers.get("content-disposition")).toMatch(
      /^attachment; filename="hs-tracker_supplier-competition_for-76_HS12-010121_V202601_scx1-[a-f0-9]{64}\.csv"$/u,
    );
  ...
  it("applies anonymous-source rate limiting inherited from Candidate Market protections", async () => {
    ...
    const request = new Request(exportUrl(), {
      headers: { "Fly-Client-IP": "198.51.100.56" },
    });
  ...
  it.each([
    {
      ...
      mutate: (url: URL) => url.searchParams.set("importer", "7600"),
      status: 400,
...
function exportUrl(
  ...
  const parameters = new URLSearchParams({
    importer: overrides.importer ?? "76",
    ...
    schema: overrides.schema ?? "supplier-competitions-csv-v1",
  });
