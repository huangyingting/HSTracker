...
} from "../../src/app/supplier-competition-export-client";
...
describe("Supplier Competition export browser client", () => {
  it("revalidates current context before creating the exact immutable URL", async () => {
    ...
    expect(prepared.url).toBe(
      `/api/v1/analyses/acceptance-fixtures-v1/supplier-competitions.csv?importer=76&product=010121&productSearchBuildId=acceptance-product-search-v3&freshnessStatusId=${encodeURIComponent(
  ...
  it("stops when the revalidated manifest no longer describes the result", async () => {
    ...
    await expect(
      ...
      name: "SupplierCompetitionExportPreparationError",
      code: "STALE_ANALYSIS",
    } satisfies Partial<SupplierCompetitionExportPreparationError>);
  });
});
